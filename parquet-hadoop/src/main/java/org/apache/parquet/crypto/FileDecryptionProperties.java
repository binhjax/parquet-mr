...
public class FileDecryptionProperties {
  ...
  private static final boolean ALLOW_PLAINTEXT_FILES = true;
  ...
  public static class Builder {
    ...
    /**
     * Set an explicit footer key. If applied on a file that contains footer key metadata - the
     * metadata will be ignored, the footer will be decrypted/verified with this key. If explicit
     * key is not set, footer key will be fetched from key retriever.
     *
    ...
    /**
     * Set explicit column keys (decryption properties). Its also possible to set a key retriever on
     * this file decryption properties object. Upon reading, availability of explicit keys is
     * checked before invocation of the retriever callback. If an explicit key is available for a
     * footer or a column, its key metadata will be ignored.
     *
    ...
    public Builder withColumnKeys(Map<ColumnPath, ColumnDecryptionProperties> columnProperties) {
      ...
      this.columnPropertyMap =
          new HashMap<ColumnPath, ColumnDecryptionProperties>(columnProperties);

      return this;
    }

    /**
     * Set a key retriever callback. It is also possible to set explicit footer or column keys on
     * this file property object. Upon file decryption, availability of explicit keys is checked
     * before invocation of the retriever callback. If an explicit key is available for a footer or
     * a column, its key metadata will be ignored.
     *
    ...
    /**
     * Skip integrity verification of plaintext footers. If not called, integrity of plaintext
     * footers will be checked in runtime, and an exception will be thrown in the following
     * situations: - footer signing key is not available (not passed, or not found by key retriever)
    ...
    /**
     * Explicitly supply the file AAD prefix. A must when a prefix is used for file encryption, but
     * not stored in file. If AAD prefix is stored in file, it will be compared to the explicitly
    ...
    /**
     * By default, reading plaintext (unencrypted) files is not allowed when using a decryptor - in
     * order to detect files that were not encrypted by mistake. However, the default behavior can
     * be overriden by calling this method. The caller should use then a different method to ensure
