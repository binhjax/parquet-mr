...
public class ColumnEncryptionProperties {
  ...
  private ColumnEncryptionProperties(
      boolean encrypted, ColumnPath columnPath, byte[] keyBytes, byte[] keyMetaData) {
    ...
    if (!encrypted) {
      ...
      if (null != keyMetaData) {
        throw new IllegalArgumentException(
            "Setting key metadata on unencrypted column: " + columnPath);
      }
    }
    if ((null != keyBytes)
        && !(keyBytes.length == 16 || keyBytes.length == 24 || keyBytes.length == 32)) {
      throw new IllegalArgumentException(
          "Wrong key length: " + keyBytes.length + ". Column: " + columnPath);
    }
  ...
  /**
   * Convenience builder for regular (not nested) columns. To make sure column name is not
   * misspelled or misplaced, file writer will verify that column is in file schema.
   *
  ...
  /**
   * Builder for encrypted columns. To make sure column path is not misspelled or misplaced, file
   * writer will verify this column is in file schema.
   *
  ...
  public static class Builder {
    ...
    /**
     * Set a column-specific key. If key is not set on an encrypted column, the column will be
    ...
    /**
     * Set a key retrieval metadata. use either withKeyMetaData or withKeyID, not both.
     *
    ...
    /**
     * Set a key retrieval metadata (converted from String). use either withKeyMetaData or
     * withKeyID, not both.
     *
