...
/**
 * This class is only required for setting explicit column decryption keys - to override key
 * retriever (or to provide keys when key metadata and/or key retriever are not available)
 */
public class ColumnDecryptionProperties {
  ...
  private ColumnDecryptionProperties(ColumnPath columnPath, byte[] keyBytes) {
    ...
    if (!(keyBytes.length == 16 || keyBytes.length == 24 || keyBytes.length == 32)) {
      throw new IllegalArgumentException(
          "Wrong key length: " + keyBytes.length + " on column: " + columnPath);
    }
  ...
  public static class Builder {
     ...
     * the metadata will be ignored, the column will be decrypted with this key. However, if the
     * column was encrypted with the footer key, it will also be decrypted with the footer key, and
     * the column key passed in this method will be ignored.
     *
