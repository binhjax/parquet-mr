...
class InternalParquetRecordReader<T> {
  ...
  private void checkRead() throws IOException {
    if (current == totalCountLoadedSoFar) {
      ...
      if (LOG.isInfoEnabled())
        LOG.info(
            "block read in memory in {} ms. row count = {}", timeSpentReading, pages.getRowCount());
  ...
  public void initialize(ParquetFileReader reader, ParquetReadOptions options) {
    ...
    System.out.println("InternalParquetRecordReader.initialize: start 1");
  ...
  public void initialize(ParquetFileReader reader, Configuration configuration) throws IOException {
    System.out.println("InternalParquetRecordReader.initialize: start 2");
    // initialize a ReadContext for this file
    this.reader = reader;
    FileMetaData parquetFileMetadata = reader.getFooter().getFileMetaData();
    this.fileSchema = parquetFileMetadata.getSchema();
